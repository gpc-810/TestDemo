package com.guo.wheelview.view;
...
import com.guo.wheelview.R;
import com.guo.wheelview.adapter.WheelAdapter;
import com.guo.wheelview.listener.LoopViewGestureListener;
import com.guo.wheelview.listener.OnItemSelectedListener;
import com.guo.wheelview.timer.InertiaTimerTask;
import com.guo.wheelview.timer.MessageHandler;
import com.guo.wheelview.timer.SmoothScrollTimerTask;
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.scale(1.0F, (float) Math.sin(radian) * SCALE_CONTENT);
                    // 控制文字倾斜角度
                    paintOuterText.setTextSkewX((textXOffset == 0 ? 0 : (textXOffset > 0 ? 1 : -1)) * (angle > 0 ? -1 : 1) *
                            DEFAULT_TEXT_TARGET_SKEWX * offsetCoefficient);
                    // 控制透明度
